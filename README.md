...
8. If using [TOTK Optimizer](https://github.com/MaxLastBreath/TOTK-mods) (which is recommended), open the file `maxlastbreath.ini` located in the `.../0100F2C0115B6000/!!!TOTK Optimizer/romfs/UltraCam` folder and change the values for `Width` and `Height` to match your screen resolution. Note: UltraCam 3.0 and above uses its own custom aspect ratio patch. To use TOTK-Ultrawide, select TOTK 2.7.1 in the `select game` dropdown menu or download an older 2.X version. 
## Pre-made mods
