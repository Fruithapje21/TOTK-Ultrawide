import os
from conversion import float2hex
...
def patch_romfs(extracted_blarc_dir, aspect_ratio, HUD_pos):
    ...
    def patch_blyt(filename, pane, operation, value):
        ...
        full_path = os.path.join(extracted_blarc_dir, 'blyt', f'{filename}.bflyt')
    ...
    def patch_anim(filename, offset, value):
        full_path = os.path.join(extracted_blarc_dir, 'anim', f'{filename}.bflan')
        with open(full_path, 'rb') as f:
            content = f.read().hex()
        idx = offset
        content_new = content[:idx] + float2hex(value) + content[idx+8:]
